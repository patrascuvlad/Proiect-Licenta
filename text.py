...
special_chars_re = re.compile('(%s)' % '|'.join(special_chars.keys()))
...
def replace_special_chars(text):
  ...
  return special_chars_re.sub(replace, text)
...
def process_text(text):
  ...
  for word in processedText:
    ...
    if not pos in posList:
      pos = 'n'
    # lemmatize word
    lWord = lmtzr.lemmatize(word[strId], pos)

    if lWord in wordsList:
      idx = wordsList.index(lWord)
      words[idx]["count"] += 1
      # max count
      maxCount = max(maxCount, words[idx]["count"])
    else:
      idx = len(words)
      wordsList.append(lWord)
      words.append({})
      words[idx]["word"] = lWord
      words[idx]["count"] = 1
